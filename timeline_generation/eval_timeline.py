...
def get_worst_mismatches(
    ...
    """
    Print the worst mismatches for false positives and false negatives.
    """
    print("Worst False Positives:")
    sorted_fp = sorted(all_false_pos.items(), key=lambda x: x[1], reverse=True)[:n]
    for patient_id, count in sorted_fp:
        print(f"Patient ID: {patient_id}, Count: {count}")

    print("\nWorst False Negatives:")
    sorted_fn = sorted(all_false_neg.items(), key=lambda x: x[1], reverse=True)[:n]
    for patient_id, count in sorted_fn:
        print(f"Patient ID: {patient_id}, Count: {count}")
