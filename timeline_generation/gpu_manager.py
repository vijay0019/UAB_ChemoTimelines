...
class GPUManager:
    ...
    def _get_gpu_utilization(self) -> Tuple[list, list]:
        ...
        with self._lock:
            ...
            dummy_util = type('obj', (object,), {'gpu': 0, 'memory': 0})()
    ...
    def _is_gpu_still_optimal(self, gpu_id: int) -> bool:
        ...
        try:
            ...
            # Consider GPU still optimal if utilization is reasonable
            gpu_util = utils[gpu_id].gpu
            memory_util = mems[gpu_id].used / mems[gpu_id].total if mems[gpu_id].total > 0 else 0
            
            # Stick with current GPU if it's not heavily loaded
    ...
    def _find_best_gpu(self) -> int:
        ...
        try:
            ...
            for i in range(self._device_count):
                ...
                # Weighted score: utilization + memory + current assignments + small random factor
                score = (
            ...
            print(f"Warning: Error in GPU selection: {e}, using round-robin")
