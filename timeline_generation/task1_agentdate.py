...
import tiktoken
import xmltodict
from typing_extensions import NamedTuple, Literal, Optional
...
Exclude surgical procedures, radiation therapy, and other non-chemotherapy-related events (including non-chemotherapy drugs).
...
Use the tool provided to help you determine the proper date format if exact date is unknown (for instance, if the text mentions "chemotherapy started four weeks ago" and the document date is 2023-10-01, use the tool: coarsen_date((2023, 10, 1, None), -28, -21) to get an approximation). Don't be afraid to only specify year and week/month (or even only year) if the exact day is not mentioned in the text.

Keep in mind that the reports are only a subset of the full timeline, so there may be events in the timeline that are not mentioned in the reports.

Again, only include chemotherapies. Exclude non-chemotherapy drugs such as trastuzumab and anastrozole.
...
[
    ...
    ('cytoxan', 'begins-on', Date(year=2011, month=8, day_of_month=1, week_of_year=None)),
    ...
    ('docetaxel', 'begins-on', Date(year=2011, month=8, day_of_month=8, week_of_year=None)),
    ('docetaxol', 'begins-on', Date(year=2011, month=8, day_of_month=8, week_of_year=None)),
    ('taxotere', 'begins-on', Date(year=2011, month=8, day_of_month=8, week_of_year=None)),
...
class ChemoTimelineBuilder(dspy.Module):
    def __init__(self):
        super().__init__()
        self.update_lm = dspy.ReAct(ChemoTimelineUpdate, tools=[coarsen_date])
