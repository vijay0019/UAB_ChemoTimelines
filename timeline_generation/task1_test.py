...
if __name__ == "__main__":
    ...
    xml_path = os.path.join(task1_path, "Gold_PairWise_EventTimex")
    ...
    for file in timeline_paths:
        if file.endswith(".json"):
            ...
            for report in sorted(os.listdir(reports_path),
                ...
                xml_file = os.path.join(xml_path, site, f"{site}_{patient}_{split}", report_name,
                                        f"{report_name}.Temporal_Relation.gold.entity_only.xml")
                temporal_relations = []
                try:
