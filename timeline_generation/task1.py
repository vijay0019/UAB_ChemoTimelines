...
Include ALL mentions from the following list (and any additional mentions found in the text):\n""" + "\n".join(sorted(CHEMO_DRUGS))
    ...
    XML: list[dict] = dspy.InputField(desc="XML annotations")
...
class ChemoTimelineBuilder(dspy.Module):
    ...
    def evaluate_and_update_timeline(self, timeline, content, xml, reasoning):
        """Process content and update timeline"""
        # Check if content is too long
        if len(tokenizer.encode(content)) > self.token_threshold:
            output = self.retry(self.debrief_lm, Notes=content, XML=xml)
            if output.get("reasoning"):
                reasoning.append(output["reasoning"])
            content = output["Timeline"]

        # Update timeline with new content
        output = self.retry(self.update_lm, previous_timeline=timeline, chunk_content=content, xml=xml)
        if output.get("reasoning"):
            reasoning.append(output["reasoning"])
    ...
    def build_generic_timeline(self, chunks, xmls, timeline, reasoning):
        """Build timeline from text chunks"""
        # Process initial chunks
        content = "\n".join(chunks[:self.starting_chunks])
        xml = xmls[:self.starting_chunks]
        timeline = self.evaluate_and_update_timeline(timeline, content, xml, reasoning)

        # Process remaining chunks
        for i in range(self.starting_chunks, len(chunks), self.intermediate_chunks):
            chunk_set = chunks[i:i + self.intermediate_chunks]
            content = "\n".join(chunk_set)
            xml = xmls[i:i + self.intermediate_chunks]
            timeline = self.evaluate_and_update_timeline(timeline, content, xml, reasoning)

        return sorted(timeline)

    def forward(self, chunks: list[str], xmls: list[dict]) -> dspy.Prediction:
        # Initialize
        timeline = []
        reasoning = []

        # Build timeline from chunks
        timeline = self.build_generic_timeline(chunks, xmls, timeline, reasoning)
...
def concatenate_chunks(data, target):
    ...
    for split in data:
        ...
        for key, value in items:
            ...
            concat = "\n\n\n".join(c for k, c, x in value)
            xmls = [x for k, c, x in value]
            keys = sorted(set(k for k, c, x in value))
            ...
            # If more than one chunk, concatenate them intelligently
                ...
                chunks = ["\n\n\n".join(c for k, c, x in value if k == i) for i in keys]
                xmls = [[x for k, c, x in value if k == i] for i in keys]
                ...
                for i, chunk in enumerate(chunks[1:], 1):
                    ...
                    if len(tokenizer.encode(current_chunk + "\n\n\n" + chunk)) < target:
                        current_chunk += "\n\n\n" + chunk
                        current_xml.append(xmls[i])
                    else:
                        concatenated_chunks.append((current_chunk, current_xml))
                        current_chunk = chunk
                        current_xml = [xmls[i]]
                if current_chunk:
                    concatenated_chunks.append((current_chunk, current_xml))
                data[split]["chunks"][key] = []
                for chunk, xml in concatenated_chunks:
                    data[split]["chunks"][key].append((chunk, xml))
                
    # Rearrange data structure
    for split in data:
        new_data = {}
        for key, (chunks, xmls) in data[split]["chunks"].items():
            if key in data[split]["timeline"]:
                new_data[key] = dspy.Example({
                    "chunks": chunks,
                    "xml": xmls,
                    "timeline": data[split]["timeline"][key]
                }).with_inputs("chunks", "xml")
        data[split] = new_data
...
if __name__ == "__main__":
    ...
    xml_path = "chemoTimelines2024_train_dev_labeled/subtask1/Gold_Pairwise_Annotations"
    ...
    for timeline in os.listdir(timelines_path):
        if timeline.endswith(".json"):
            ...
            for report in sorted(os.listdir(reports_path), key=lambda x: ['NOTE', 'PGN', 'RAD', report_type(x)].index(report_type(x))):
                ...
                try:
                    with open(os.path.join(xml_path, site, split, f"{site}_{patient}_{split}", report_id, f"{report_id}.Temporal_Relation.gold.inprogress.xml"), 'r') as f:
                        xml_content = f.read().strip()
                    xml = xmltodict.parse(xml_content)
                except FileNotFoundError:
                    print(f"XML file not found for {report} in {reports_path}")
                    xml = {}
                data[split]["chunks"][f"{site}_{patient}"].append((re.search(r'report(\d+)', report).group(1), content, xml))
